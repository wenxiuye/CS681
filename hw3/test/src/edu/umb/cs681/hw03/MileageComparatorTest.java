...
class MileageComparatorTest {
	...
	@Test
	void sortCarByMileageAscendingOrder() {
		...
		//get sorted car list using lambda expression
		ArrayList<Car> sortedCars = (ArrayList<Car>) usedCar.stream()
															.sorted((Car car1, Car car2)-> car1.getMileage() - car2.getMileage())
															.collect(Collectors.toList());
	
		
		//Collections.sort(usedCar, new MileageComparator<Car>());
		
		//create a list of cars with lower to higher mileage
		...
		//for (String s : actualSortedUsedCarList)
	...
	void findMedian() {
		...
		int median = sortedCars.size() % 2 == 1 ? 
				sortedCars.get(sortedCars.size()/2).getMileage() : 
					(sortedCars.get(sortedCars.size()/2 - 1).getMileage() + sortedCars.get(sortedCars.size()/2).getMileage()) /2 ;
		
		assertEquals(47406, median);
