...
class PriceComparatorTest {
	...
	@Test
	void sortCarByPriceAscendingOrder() {
		...
		//lower to higher
		//Collections.sort(usedCar, new PriceComparator<Car>());
	...
	void findMedian() {
		...
		//find the median
		float median = sortedCars.size() % 2 == 1 ? 
				sortedCars.get(sortedCars.size()/2).getPrice() : 
					(sortedCars.get(sortedCars.size()/2 - 1).getPrice() + sortedCars.get(sortedCars.size()/2).getPrice()) /2 ;
		
		assertEquals(17947.5, median);
